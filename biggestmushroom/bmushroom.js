function setup() {
  ...
  push();
  translate(600, 900);                 // 整个蘑菇的位置
  drawStemUniform();                   // 白色伞柄
  drawCapReplica(0, -650, 880, 360);   // 伞盖：中心 x, y, 宽, 高
  pop();
}
...
/* 保存图片（可选） */
function keyPressed() {
  if (key === 's' || key === 'S') saveCanvas('mushroom_kusama_like', 'png');
}
