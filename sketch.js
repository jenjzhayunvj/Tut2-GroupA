...
const PatternPainter = {
  ...
  baseTracks(deps, cfg = {}) {
    ...
    for (let step = 0; step <= totalSteps; step++) {
      ...
      let trackColor = useAlt ? (trackIndex % 2 === 0 ? c1 : c2) : c1;// choose color based on track index, if usealt and odd, use c2, else c1
      fill(trackColor);
      for (let i = 1; i <= circlesPerTrack; i++) {
        let r = radialStep * i;
        let x = center.x + r * cos(angle);// cosine for x
        let y = center.y + r * sin(angle);/// sine for y
  ...
  stemVoronoi(deps, opt = {}) {
    ...
    let satRange = opt.satRange ?? 10;
    let briRange = opt.briRange ?? 10;
  ...
  stemDotTracks(deps, opt = {}) {
    ...
    let stepY = height / (rows + 1); // rows +1 to leave some gap at top and bottom

    for (let k = 0; k < trackCount; k++) {// from left to right
      let t = (k + 0.5) / trackCount;
      let x = lerp(bb.minx + marginX, bb.maxx - marginX, t);

      // dots in the center are larger, dots near the edges are smaller
      let centerT = 0.5;
      let dist = abs(t - centerT) / centerT;
      let scale = lerp(1.0, edgeScale, dist);

      for (let i = 1; i <= rows; i++) { // from top to bottom
        let yBase = bb.miny + stepY * i;
        let y = yBase + random(-jitterY, jitterY);
...
// ---------- Cap class ----------
// Top part of a small mushroom: curved outline + decorative pattern
class Cap {
  constructor(spec) {
  ...
  buildGeometry() {
    // Build cap polygon, colors and cached geometry from spec / random values
...
// ---------- Stem class ----------
// Middle part of a small mushroom: vertical body with optional pattern
class Stem {
  constructor(spec) {
    this.visible = spec.visible != null ? spec.visible : true;
  ...
  draw() {
    ...
    // Draw optional outline and then clip pattern inside stem bounds
...
// ---------- BasePart class ----------
// Bottom plate under the stem: optional support with its own pattern
class BasePart {
  constructor(spec) {
  ...
  draw() {
    ...
    // Clip the base shape and fill it with its radial track pattern if enabled
    withClip((ctx2) => this.path(ctx2), () => {
      const bounds = {
        x: -this.w * 0.5,
        y: -this.h * 0.5,
...
// ---------- Mushroom class ----------
// Combines cap, stem and base into a single positioned small-mushroom object
class Mushroom {
  constructor(spec) {
    this.id = spec.id || "m";
    this.seed =
    ...
    // Create three sub-parts from the merged spec: cap, stem and base plate
  ...
  draw() {
    // Use per-mushroom seeds so random-based patterns stay stable per instance
    ...
    // Resolve world positions for each part from anchor + layout offset or overrides
    ...
    // Draw base, stem, then cap in back-to-front order
...
// Type library
// Template specs for reusable small-mushroom styles (baseline parts + patterns)
const TYPE_LIBRARY = {
  default: {
    cap: {
      visible: true,
...
// Place each small mushroom instance in the scene and override its part styles
const SCENE_LAYOUT = [
  {
    id: "m_greenL",
    type: "default",
...
// Build a Mushroom from a type template plus per-instance overrides for each part
function makeMushroomFromLayout(layout) {
  ...
  // Merge base type spec with layout overrides for cap, stem, and base
  ...
  // Pass the merged part specs and layout into the Mushroom constructor
  return new Mushroom({
    id: layout.id,
    seed: layout.seed,
    anchor,
...
function setup() {
  ...
  // Build all small mushroom instances from the scene layout specs
  mushrooms = [];
...
function draw() {
  ...
  // Small mushrooms: draw each composed mushroom object (cap + stem + base)
